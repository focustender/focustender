...
**Home and Ceramics** can be previewed by just opening the `.html` file in
a browser, or with a VS Code Live Server-style extension — all asset paths
are relative, no dev server required.

**Work and Design cannot** — their content (résumé PDF link, masonry
images) is generated by `import.meta.glob`, a Vite-only build-time
feature. Opened through a plain static server, `src/work.js`/`design.js`
fail immediately (`import.meta.glob is not a function` — it isn't real
browser JS, Vite rewrites it during its own processing), so those two
pages render with an empty content area and no error visible on the page
itself. This is the #1 likely cause if photos/résumé "don't show up" —
check the browser console for that error to confirm. Two ways to
correctly preview them:

1. `npm run dev` — Vite's own dev server (recommended for regular
   development; instant reload, matches production).
2. `npm run build`, then point Live Server (or any static server) at
   `dist/` instead of the project root. The build resolves
   `import.meta.glob` into real file references ahead of time, so the
   compiled output works fine under any plain static server — this is
   also exactly what GitHub Pages serves in production.
